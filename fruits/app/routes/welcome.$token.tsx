...
async function loadInvitedHuman(
  ...
  if (!human || !isInviteTokenValid(human)) {
    ...
    // `redirect("/login")` here would immediately re-redirect to the
    // dashboard for a signed-in user, with zero indication of what happened.
    throw redirect(
      existingUser ? "/profile?inviteExpired=1" : "/login",
    );
  }
  return human;
}
...
function PasskeySetup({
  ...
  return (
    <AuthShell title={`Welcome, ${invitedName}`}>
      <div
        className={`${surfaceBase} ${sprinkles({
          display: "flex",
          flexDirection: "column",
