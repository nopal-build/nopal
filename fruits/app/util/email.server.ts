// DUPLICATED from webapp/app/util/email.server.ts -- both apps send real
// email (this one: login codes, invites, profile changes; webapp: contact
// form, WC waiver), and this stayed out of the shared robustness-core
// package on purpose (no React/email-provider deps anywhere else in it --
// see the auth.server.ts `configureAuth` seam in
// packages/robustness-core/src/auth/ for the same reasoning). Not shared;
// keep both in sync by hand if you touch how mail actually gets sent.
