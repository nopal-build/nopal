...
The product app (`o.nopal.build`) — Daily Log, Vault, Maker, Projects,
login/passkeys. Split out from `webapp` (the marketing site, `nopal.build`)
— see `docs/marketing-app-split-plan.md` for the history and rationale.
...
shared with `webapp` too — see the UI conventions below.

A few files under `app/components`, `app/hooks`, and `app/util` (`Layout`,
`Footer`, `GoodAssets`, `useSchemePref`, `useClickOutside`, `email.server`)
are deliberate **duplicates** of the same-named files in `webapp` — see
each one's own top-of-file comment before assuming a change here should
also happen there (or vice versa). Everything else under `app/` is
app-native, not shared with `webapp` at all.
...
- Always use the shared components from `packages/stamps` (e.g. `Input`,
  ...
  mentioned above, not app-native, despite living in this same folder.
...
- Run `npx tsc --noEmit -p .` (or `npm run typecheck`) from `fruits/` after
