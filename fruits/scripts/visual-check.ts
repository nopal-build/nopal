// fruits/scripts/visual-check.ts
//
...
//
// Requires:
//   - `npm run dev` already running (this script can't start it itself --
//     it's a long-lived server). Defaults to http://localhost:3001 -- the
//     fruits service's own docker-compose port -- override with
//     VISUAL_CHECK_BASE_URL if you're running it some other way.
...
const BASE_URL = process.env.VISUAL_CHECK_BASE_URL ?? "http://localhost:3001";
