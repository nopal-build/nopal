...
- A file attachment (`::file{...}`) inside a Card — ACTUALLY filed into the project (`copyFileIntoFolder`, `vault.server.ts`): a new `file_refs` row in the project's own folder pointing at the exact same S3 bytes, no duplication. Cross-human safe (a Card can target a project shared with the acting human via any Sharing Role, including Observer — see `getAccessibleProjectFolders`). This is the ONE signal kind that produces a real changeset (see below), and so the only kind that can be reverted.
