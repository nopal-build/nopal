...
**A real, explicitly-flagged limitation**: "replay = reapply the stored snapshot" is only guaranteed correct for INDEPENDENT operations — true of every changeset produced today (each `"created"` changeset is a brand new file, never touched by any other entry). Once a future AI-driven filing step produces chained `content-edit` changesets on the SAME file (e.g. successive README rewrites), a later edit might have been generated assuming an earlier one was already applied, and blindly reapplying its stored `after` snapshot post-revert could reintroduce or duplicate content. Not solved here on purpose — flagged so it's a known problem to revisit, not rediscovered from scratch, once that step exists.
