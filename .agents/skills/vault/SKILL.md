...
- **`directive-attr`** — a `file="..."`/`folder="..."` attribute on another leaf directive (`::csv-table{file=...}`, `::gallery{folder=...}`), resolved against the attribute's own containing folder's direct children by name — mirrors `project.server.ts`'s `resolveProjectManifest`.
