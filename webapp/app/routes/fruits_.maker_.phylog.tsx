// app/routes/fruits_.maker_.phylog.tsx
// PhyLog usage deep-dive — linked from /fruits/maker's summary section.
// Admin/Super only, same gate as the parent Maker dashboard.
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const usage = await getPhylogUsageSummary(days);
        ...
        <div className="container mx-auto px-4 py-12" style={{ maxWidth: "480px" }}>
          <div className="good-box p-6 flex flex-col gap-3">
            ...
            <p className="text-sm subtle-text">
              PhyLog usage stats are only available to Admin and Super accounts.
            </p>
    ...
    <div className="flex gap-2">
      {([7, 30] as const).map((option) => {
        ...
        return (
          <Link
            key={option}
            to={`/fruits/maker/phylog?range=${option}`}
            prefetch="intent"
...
const STAGE_LABELS: Record<PhylogStage, string> = {
      ...
      <div className="container mx-auto px-4 py-12" style={{ maxWidth: "860px" }}>
        ...
        <section className="mb-12">
          ...
          <div className="good-box p-5">
            {(Object.keys(STAGE_LABELS) as PhylogStage[]).map((stage) => {
        ...
        <section className="mb-12">
          ...
          <div className="good-box p-5">
            {usage.byDate.length === 0 ? (
              <p className="text-sm subtle-text">No PhyLog activity in this range.</p>
            ) : (
              usage.byDate.map((d) => (
                <BarRow
                  key={d.date}
