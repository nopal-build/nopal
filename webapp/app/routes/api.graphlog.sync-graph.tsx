...
/**
 * POST /api/graphlog/sync-graph
 *
 * Enqueues GraphLog's sync-graph stage — see the `graphlog` skill.
 * Agentic (real LLM calls), so this follows PhyLog's own enqueue-then-poll
 * shape (same as `api.graphlog.sync-knowledge.tsx`). Returns immediately
 * with a job id; poll `GET /api/graphlog/jobs/:jobId` for progress/
 * results. Thin client: `nopal graphlog sync-graph`.
 *
 * Body:
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    const jobId = await enqueueGraphLogJob("sync-graph", {
      actingHumanId: user._id,
      projectFolderId,
    });
    return Response.json({ jobId }, { status: 202 });
  } catch (err) {
    console.error("GraphLog sync-graph enqueue error:", err);
    return Response.json(
      { error: err instanceof Error ? err.message : "Failed to enqueue sync-graph" },
      { status: 500 },
    );
  }
}
