 ...
 * matter, the README's body (rendered as-is via MdxEditorView), and every
 * file/subfolder referenced by a `file="..."`/`folder="..."` attribute on a
 * directive in that body, fetched and shaped into plain JSON-serializable
 * Records so it can be returned directly as loader data.
 *
...
/**
 * Returns `null` when the folder has no `README.md`, or the README has no
 * (valid) manifest front matter — callers should fall back to the plain
 * vault folder view in that case, never a dead end.
 */
export async function resolveProjectManifest(
  ...
  if (!readmeListing) return null;

  const readme = await getFileRefById(readmeListing._id);
  if (!readme?.content) return null;

  const { manifest, body } = parseProjectManifest(readme.content);
  ...
  return { manifest, body, files, folders, csvFields };
}
