type WaiverCompleteProps = {
  icName: string;
  contractorName: string;
  signedAt: string;
  pdfUrl: string;
  recipientType: "admin" | "contractor";
};

export function WaiverComplete({
  icName,
  contractorName,
  signedAt,
  pdfUrl,
  recipientType,
}: WaiverCompleteProps) {
  const date = new Date(signedAt).toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });

  return (
    <div
      style={{
        color: "#3f2b46",
        margin: "16px auto",
        maxWidth: "600px",
        padding: "16px 0",
        fontFamily: "SF Pro Text, -apple-system, sans-serif",
      }}
    >
      <div
        style={{
          marginBottom: "16px",
          display: "flex",
          alignItems: "flex-end",
          justifyContent: "space-between",
        }}
      >
        <a href="https://nopal.build" target="_blank" rel="noopener noreferrer">
          <img
            src="data:image/svg+xml;base64,"
            alt="nopal"
            width="131"
            height="58"
            style={{ display: "block" }}
          />
        </a>
        <div style={{ fontStyle: "italic", fontSize: "14px" }}>
          <p>We build for health.</p>
        </div>
      </div>

      <div
        style={{
          background: "#fff9f1",
          border: "1px solid #e5d6c5",
          padding: "24px",
          borderRadius: "8px",
        }}
      >
        <h2
          style={{
            margin: "0 0 16px",
            color: "#3f2b46",
            fontSize: "20px",
          }}
        >
          {recipientType === "admin"
            ? "Workers' Comp Waiver Completed"
            : "Your Workers' Comp Waiver is Signed"}
        </h2>

        <p style={{ margin: "0 0 12px", lineHeight: "1.6" }}>
          {recipientType === "admin" ? (
            <>
              <strong>{icName}</strong> has completed and signed the Arizona
              Independent Contractor Workers' Compensation Acknowledgment for{" "}
              <strong>{contractorName}</strong> on {date}.
            </>
          ) : (
            <>
              Thank you, <strong>{icName}</strong>. Your signed Workers'
              Compensation Acknowledgment for <strong>{contractorName}</strong>{" "}
              has been recorded on {date}. A copy of your signed document is
              attached to this email.
            </>
          )}
        </p>

        <p style={{ margin: "0 0 16px", lineHeight: "1.6" }}>
          The signed PDF is attached to this email. You can also{" "}
          <a
            href={pdfUrl}
            style={{ color: "#5da06d" }}
            target="_blank"
            rel="noopener noreferrer"
          >
            view or download it here
          </a>
          .
        </p>

        {recipientType === "contractor" && (
          <p
            style={{
              margin: "0",
              fontSize: "13px",
              color: "#817186",
              lineHeight: "1.5",
            }}
          >
            Please retain this document for your records. If you have any
            questions, contact us at{" "}
            <a href="mailto:human@nopal.build" style={{ color: "#5da06d" }}>
              human@nopal.build
            </a>
            .
          </p>
        )}
      </div>

      <p
        style={{
          marginTop: "16px",
          fontSize: "12px",
          color: "#817186",
          lineHeight: "1.5",
        }}
      >
        This document was electronically signed in accordance with the federal
        Electronic Signatures in Global and National Commerce Act (ESIGN) and
        the Uniform Electronic Transactions Act (UETA).
      </p>
    </div>
  );
}
