...
/**
 * How a login code's email actually gets rendered + sent is deliberately
 * kept OUT of this shared module -- rendering it (a React email component)
 * and sending it (Resend/SMTP) are app-level concerns this package has no
 * other reason to depend on (no React, no email provider SDKs anywhere
 * else in robustness-core). Whichever app owns the login flow (today:
 * webapp; going forward: the app service at o.nopal.build -- see
 * docs/marketing-app-split-plan.md) MUST call `configureAuth` once, before
 * any real login attempt, wiring this up with its own `sendEmail`/email
 * template. See webapp/app/modules/auth/auth.server.ts for that wiring --
 * it's TEMPORARY there, only until login itself moves to the app service.
 */
export type SendTotpEmailParams = {
  email: string;
  code: string;
  magicLink: string;
};
export type SendTotpEmail = (params: SendTotpEmailParams) => Promise<void>;

let sendTotpEmailImpl: SendTotpEmail | null = null;

/** Safe to call more than once (e.g. a dev-server hot reload) -- the last call wins. */
export function configureAuth(options: { sendTotpEmail: SendTotpEmail }): void {
  sendTotpEmailImpl = options.sendTotpEmail;
}
  ...
  new TOTPStrategy(
    {
      ...
      sendTOTP: async ({ email, code, magicLink }) => {
        if (!sendTotpEmailImpl) {
          throw new Error(
            "configureAuth({ sendTotpEmail }) must be called before login is attempted -- see webapp/app/modules/auth/auth.server.ts.",
          );
        }
        await sendTotpEmailImpl({ email, code, magicLink });
      },
    },
    async ({ email, request }) => {
