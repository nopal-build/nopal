...
// `webapp/app/components/OxEditor.tsx`/`OxRenderer.tsx`) and the PhyLog
// pipeline (`robustness-core`'s `dailyLog`/`sorter`/`fileReferences`/
...
// because it's consumed by BOTH the web app and the PhyLog worker, so it
// can't live inside either one without the other reaching across a
